...
The Target Environment: An isolated application context where workloads are given explicitly tuned CPU bounds.
The Chaos Experiment (Blueprint): A read-only template outlining container-runtime variables and execution args.
The Chaos Engine (Trigger Contract): Intercepts specific application workloads via label selectors and injects live tunable overrides (e.g., duration, core consumption).
